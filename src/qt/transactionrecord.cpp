...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        // <TYM
            const int nDevPaymentsStartBlock = 250001;
            ...
            int nBlockHeight = pindex ? pindex->nHeight : 0;

            // Masternode reward
            CTxDestination destMN;
            int nIndexMN = wtx.vout.size() - (nBlockHeight >= nDevPaymentsStartBlock ? 2 : 1);
            ...
            // Dev reward
            if (nBlockHeight >= nDevPaymentsStartBlock) {
                CTxDestination dest;
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by timelockcoin Address
