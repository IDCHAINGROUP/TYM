...
#if defined(NDEBUG)
#error "timelockcoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in utimelockcoin) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateTYMSupply(int nHeightStart)
{
	AssertLockHeld(cs_main);
	...
	uiInterface.ShowProgress(_("Recalculating TYM supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating TYM supply..."), percent);
		}
...
CAmount GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        nSubsidy = 0 * COIN;
    } else if (nHeight == 1 ) {         // Premine:     800 000 TYM
        nSubsidy = 100000 * COIN;
    } else if (nHeight <= 1000) {        // 0 - 500:     2.0 TYM
        nSubsidy = 2500 * COIN;            
    } else if (nHeight <= 10000) {      // 501 - 30000: 0.5 TYM, ...
        nSubsidy = 0.052 * COIN;
        ...
        nSubsidy = 30.002 * COIN;
    } else if (nHeight <= 100000) {
        nSubsidy = 25.002 * COIN;
    } else if (nHeight <= 150000) {
        nSubsidy = 20.002 * COIN;
    } else if (nHeight <= 160000) {
        nSubsidy = 15.002 * COIN;
    } else if (nHeight <= 210000) {
        nSubsidy = 10.002 * COIN;
    } else if (nHeight <= 250000) {
        nSubsidy = 5.002 * COIN;
    } else if (nHeight <= 260000) {
        nSubsidy = 100.0001 * COIN;
    } else if (nHeight <= 300000) {
        nSubsidy = 55.0001 * COIN;
    } else if (nHeight <= 310000) {
        nSubsidy = 30.0001* COIN;
    } else if (nHeight <= 350000) {
        nSubsidy = 25.0001 * COIN;
    } else if (nHeight <= 360000) {
        nSubsidy = 20.0001 * COIN;
    } else if (nHeight <= 400000) {
        nSubsidy = 15.0001 * COIN;
    } else {
        nSubsidy = 10.0001 * COIN;
    }
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    if (nHeight <= 1000) {        // 0 - 500:     2.0 TYM
        ...
        mnSubsidy = 30.00 * COIN;
    } else if (nHeight <= 100000) {
        mnSubsidy = 25.00 * COIN;
    } else if (nHeight <= 150000) {
        mnSubsidy = 20.00 * COIN;
    } else if (nHeight <= 160000) {
        mnSubsidy = 15.00 * COIN;
    } else if (nHeight <= 210000) {
        mnSubsidy = 10.00 * COIN;
    } else if (nHeight <= 250000) {
        mnSubsidy = 5.00 * COIN;
    } else if (nHeight <= 260000) {
        mnSubsidy = 50.00 * COIN;
    } else if (nHeight <= 300000) {
        mnSubsidy = 30.00 * COIN;
    } else if (nHeight <= 310000) {
        mnSubsidy = 16.00 * COIN;
    } else if (nHeight <= 350000) {
        mnSubsidy = 15.00 * COIN;
    } else if (nHeight <= 360000) {
        mnSubsidy = 14.00 * COIN;
    } else if (nHeight <= 400000) {
        mnSubsidy = 10.00 * COIN;
    } else {
        mnSubsidy = 5.50 * COIN;
...
CAmount GetDevPayment(int nHeight, CAmount blockValue)
{
    ...
    if (nHeight <= 250000) {
        ...
        nDevPayment = 50.000000001 * COIN;  // 0.1 satoshi to prevent rounding issues, it is stripped down inside TX
    } else if (nHeight <= 300000) {
        nDevPayment = 25.000000001 * COIN;
    } else if (nHeight <= 310000) {
        nDevPayment = 14.000000001 * COIN;
    } else if (nHeight <= 350000) {
        nDevPayment = 10.000000001 * COIN;
    } else if (nHeight <= 360000) {
        nDevPayment = 6.000000001 * COIN;
    } else if (nHeight <= 400000) {
        nDevPayment = 5.000000001 * COIN;
    } else {
        nDevPayment = 4.500000001 * COIN;
    }
...
{
    util::ThreadRename("timelockcoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // <TYM
    const int dev_enforce_block = 251000;
    ...
    if (pindex->nHeight > last_pow_block) {
      ...
      if (nDevReward > 0) {
        ...
        if (!DevPaid) {
            if (pindex->nHeight < dev_enforce_block) {
                LogPrintf("Warning: ConnectBlock(): no dev reward");
            } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> timelockcoinInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                timelockcoinInputs.push_back(stakeIn);
        }
        const bool hasTYMInputs = !timelockcoinInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasTYMInputs) {
                    ...
                    for (const CTxIn& timelockcoinIn : timelockcoinInputs)
                        if(timelockcoinIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasTYMInputs) {
                            for (const CTxIn& stakeIn : timelockcoinInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // timelockcoin: We use certain sporks during IBD, so check to see if they are
